...
class Learner:
    ...
    def learn_from_query(self, query, retrieved_context):
        ...
        new_domain_matches = re.search(r"<new>(.*?)<\new>", resp, re.DOTALL).group(1).strip()
        ...
        fact_matches = re.search(r"<l>(.*?)<\l>", resp, re.DOTALL).group(1).strip()
        ...
        for fact in facts_to_add:
            ...
            if domain not in domain_fact:
                domain_fact[domain] = []
            domain_fact[domain].append(factoid)
        ...
        for top_level_domain, new_facts_list in domain_fact.items():
            ...
            for change in changes:
                x, field, fact_string = change.split("|")
                self.mem.change_subfield_and_fact(top_level_domain, field, fact_string, (x.lower()=="add"))
